...
# MAX_PENALTY и формула ниже сознательно пересчитаны (см. ROADMAP.md,
# R1) - раньше потолок был 15, а сама формула (deficit/2) на РЕАЛЬНОМ
# (не катастрофическом) win-rate хронически слабой стратегии вроде
# Donchian Breakout (~21-30%, не 0%) выдавала штраф всего +5..+9 -
# даже близко не доходя до старого потолка. При этом HTF-подтверждение
# (multi_timeframe.CONFLUENCE_BONUS_PER_TF=8 * 3 таймфрейма) может
# добавить к score до +24 - штраф в +5..+9 против буста в +24 ничего
# структурно не решал, стратегия с плохой статистикой всё равно легко
# проходила порог при любом подтверждении старших ТФ.
#
# Новые значения: формула стала линейной (без /2) - тот же deficit
# теперь даёт вдвое больший штраф (при win-rate~21% - уже +19, а не
# +9). Потолок поднят до 25 - выше типичного HTF-буста (+24), но
# оставляет config.MIN_SIGNAL_SCORE_TO_PUBLISH(=70) + 25 = 95, то есть
# 5-очковый зазор до максимума в 100 - сигнал технически ВСЁ ЕЩЁ может
# пройти при исключительном score, автоматика по-прежнему не блокирует
# стратегию полностью (тот же принцип, что и раньше - см. ниже).
MAX_PENALTY = 25                 # потолок штрафа (score points) - не блокируем стратегию полностью
...
def recompute_adjustments(days: float = TUNING_LOOKBACK_DAYS) -> dict:
    ...
    for strategy, s in stats.get("by_strategy", {}).items():
        ...
        if s["win_rate"] < WEAK_WIN_RATE_THRESHOLD:
            # Линейно (см. комментарий у MAX_PENALTY выше про R1) - чем
            # сильнее win-rate ниже порога слабости, тем строже штраф,
            # но не более MAX_PENALTY (сознательно не даём автоматике
            # полностью заблокировать стратегию без участия человека -
            # для этого есть ручной MIN_SIGNAL_SCORE_TO_PUBLISH).
            deficit = WEAK_WIN_RATE_THRESHOLD - s["win_rate"]
            penalty = min(MAX_PENALTY, round(deficit))
            if penalty > 0:
