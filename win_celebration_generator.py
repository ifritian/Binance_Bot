...
По референсу пользователя (пост другого автора на Binance Square) тон
здесь ещё и более "тёплый вживую" - с эмодзи и адресной благодарностью
подписчикам ("to everyone who stayed with me" в оригинале), а не только
безличная радость за сделку. Отсюда:
- _ALLOWED_EMOJI/промпт разрешают 1-3 эмодзи, но только из "тёплого"
  набора (радость/благодарность), НЕ из спекулятивно-финансового
  (🚀 "на луну", 💰/💵 "деньги") - такие эмодзи по смыслу равны словесному
  обещанию будущей прибыли, а это уже запрещено самим форматом (см.
  СТРОГО ЗАПРЕЩЕНО ниже) - лучше не давать модели лазейку через эмодзи
  для того, что запрещено текстом.
- validate_win_celebration_hook ниже блокирует и спам-набор эмодзи
  (больше 4 в хуке - это уже не живая реакция, а перегруз), и сами
  спекулятивные эмодзи - на случай, если модель их всё равно вставит.
...
_ANGLES = {
    ...
    "gratitude": (
        "тёплая, адресная благодарность подписчикам - тем, кто был рядом "
        "с самого входа в сделку и никуда не делся, пока она шла: "
        "по духу как \"спасибо, что оставались со мной всё это время\", "
        "а не общее \"спасибо за внимание\""
    ),
}

# Эмодзи, которые можно использовать в хуке - только тёплые/эмоциональные,
# без намёка на прогноз или денежную сумму (см. docstring модуля).
_ALLOWED_EMOJI = "🔥🙌🎉😄🥹🙏❤️✨🫶"
# Спекулятивные/финансовые эмодзи - недопустимы по тем же причинам, что и
# словесные обещания будущей прибыли (см. СТРОГО ЗАПРЕЩЕНО в промпте).
_BANNED_EMOJI = "🚀💰💵📈💸🤑"
_MAX_EMOJI_IN_HOOK = 4

_EMOJI_RE = re.compile(
    "[" + re.escape(_ALLOWED_EMOJI + _BANNED_EMOJI) + "]"
)
_BANNED_EMOJI_RE = re.compile("[" + re.escape(_BANNED_EMOJI) + "]")
...
Добавь 1-3 эмодзи, органично по тексту (не все подряд в одном месте) -
из тёплого/радостного набора: 🔥 🙌 🎉 😄 🥹 🙏 ❤️ ✨ 🫶. Это не "текст плюс
эмодзи-строка в конце", а часть живой реакции, как пишут в реальных
соцсетях.
...
- эмодзи вроде 🚀💰💵📈 - они читаются как обещание будущей прибыли, а не
  как эмоция от уже закрытой сделки.
...
def validate_win_celebration_hook(hook: str) -> tuple:
    ...
    Дополнительно (см. docstring модуля про тон по референсу пользователя):
    - блокирует спекулятивные/финансовые эмодзи (🚀💰 и т.п.) - по смыслу
      это обещание будущей прибыли, только через эмодзи, а не текст;
    - блокирует перебор эмодзи вообще (> _MAX_EMOJI_IN_HOOK) - это уже
      выглядит спамом, а не живой реакцией."""
    ...
    if _BANNED_EMOJI_RE.search(hook):
        found = sorted(set(_BANNED_EMOJI_RE.findall(hook)))
        return False, f"В хуке есть спекулятивные/финансовые эмодзи (похоже на обещание прибыли): {', '.join(found)}"

    emoji_count = len(_EMOJI_RE.findall(hook))
    if emoji_count > _MAX_EMOJI_IN_HOOK:
        return False, f"В хуке слишком много эмодзи ({emoji_count}) - выглядит спамом, а не живой реакцией"
