...
ВАЖНО (см. README_OKX1.md): у OKX Orbit нет публичного API для
публикации - в отличие от Binance Square, эти посты НЕ публикуются
автоматически. okx_draft_publisher.py отправляет готовый текст (и
картинку, если есть) владельцу в Telegram, публикация - вручную через
приложение OKX. Именно поэтому здесь нет ни $CASHTAG-механики, ни
Binance-хэштегов из post_format.py - это чужая площадка со своими
правилами дискаверабилити, которых мы не знаем наверняка, поэтому текст
максимально нейтральный и самодостаточный.
...
1. "market_take" - развёрнутое наблюдение/мнение о движении рынка
   (аналог opinion_generator, но без Binance-специфики).
2. "trading_insight" - короткое, конкретное наблюдение по сетапу/уровню
   без явного сигнала на сделку (аналог hot_take_generator, но без
   контрарной провокационной рамки - OKX хочет "trading insights", а
   не намеренный холивар).

Реальные числа (% изменения, амплитуда) считаются кодом через
opinion_generator.calc_theme_stats (тот же источник данных, что и у
Binance-бота) - LLM получает готовые цифры и пишет вокруг них, как и
...
def pick_format(last_format: Optional[str]) -> str:
    ...
    принцип ротации, что и opinion_generator.pick_theme."""
    import random
...
def generate_okx_orbit_post(theme: str, format_type: str) -> Optional[tuple]:
    ...
    В отличие от generate_opinion_post/generate_hot_take - НЕ вызывает
    assemble_post() (та функция заточена под Square-специфичные CTA/
    хэштеги), собирает финальный текст сам: хук + DISCLAIMER.
    """
...
def validate_okx_orbit_post_text(text: str, allowed_numbers: set[float]) -> tuple[bool, str]:
    """Те же проверки, что у validate_opinion_post_text - числа только
    из посчитанных данных, дисклеймер на месте, без ИИ-штампов."""
    import re
