...
# независимо от формата - поверх индивидуальных таймеров каждого
# формата (MIN_POST_INTERVAL_HOURS у сигналов, OPINION_INTERVAL_HOURS и
# т.д.). У каждого формата свой таймер, но они друг про друга не знают -
# без этой проверки два разных формата (например, сигнал и Treasury
# Index) могут технически выйти в Square с разницей в пару минут просто
# по случайному совпадению джиттеров, и такие посты буквально отъедают
# охват друг у друга (тот же пул подписчиков видит их почти одновременно,
# внимание/показы делятся). Проверяется через
# queue_manager.seconds_since_last_post("square_global") - см. main.py,
# _square_slot_available() и каждую try_publish_*, которая постит в
# Square. Binance не публикует точный порог для этого - оценка на основе
# общих паттернов (диминишинг ретёрнс, самоконкуренция за один и тот же
# пул подписчиков при кластеризации постов), настраивается по факту
# реальных данных просмотров.
MIN_SQUARE_POST_SPACING_MINUTES = float(os.environ.get("MIN_SQUARE_POST_SPACING_MINUTES", "45"))
...
# --- Новостной формат "мнение по новости" (см. news_opinion_generator.py) ---
# Публичный Telegram-канал, чей t.me/s/<канал> превью-страница читается
# без какого-либо аккаунта. @forklog - независимое крипто-медиа.
NEWS_SOURCE_CHANNEL = os.environ.get("NEWS_SOURCE_CHANNEL", "forklog")
NEWS_TAKE_ENABLED = os.environ.get("NEWS_TAKE_ENABLED", "false").lower() == "true"
...
# --- P3.8: ATR вместо фиксированного "измеренного движения" для цели
# (target) в strategies.build_macd_signal/build_breakout_signal ---
# Сейчас (по умолчанию, USE_ATR_TARGETS=false): target считается по
# СТРУКТУРЕ - для MACD Crossover это ближайший локальный экстремум
# последних 20 свечей (recent_high/recent_low), для Donchian Breakout -
# классическое "измеренное движение" (высота пробитого канала,
# отложенная от уровня пробоя). Проблема та же, что была у фиксированного
# % для стопа (см. A2 выше): структура канала/экстремума формировалась
# в КАКОМ-ТО прошлом режиме волатильности, который может уже не иметь
# отношения к текущему - на резко успокоившемся рынке измеренное
# движение может оказаться нереалистично широким относительно того, что
# цена пройдёт за разумное время, а на резко разогнавшемся - наоборот,
# заниженным относительно текущего потенциала хода.
#
# Когда USE_ATR_TARGETS=true: target = entry ± ATR_TARGET_MULTIPLIER *
# ATR(ATR_PERIOD) (см. strategies._atr_target, calc_atr) - от ТЕКУЩЕЙ
# цены входа, а не от структурного уровня. Если ATR посчитать не
# удалось (свечей мало) - тихо используется СТАРАЯ формула (экстремум/
# измеренное движение) как запасной вариант, тот же принцип отката, что
# и у A2. НЕ проверяет R:R сама - это по-прежнему делает downstream-
# фильтр scanner.MIN_RISK_REWARD_RATIO, который отсеет сигнал, если
# получившийся target даёт R:R хуже порога, независимо от того, как
# именно target был посчитан.
#
# ATR_TARGET_MULTIPLIER=3.0 при дефолтном ATR_STOP_MULTIPLIER=1.5 (A2
# выше) даёт R:R около 1:2.0 даже когда ОБА флага включены одновременно -
# с запасом выше MIN_RISK_REWARD_RATIO=1.2 по умолчанию. Множители
# независимы (можно включить только один из двух флагов) - это
# сознательный выбор дефолта на случай, если включены оба сразу, а не
# жёсткая связь между ними.
#
# ПО УМОЛЧАНИЮ ВЫКЛЮЧЕНО, как и A2 - меняет саму формулу цели для уже
# работающих стратегий. Роадмап требует бэктеста до/после, прежде чем
# включать на реальных деньгах (см. backtest.py --use-atr-targets) - не
# переключайте на true, не сравнив хотя бы aggregate_report до и после
# на одних и тех же исторических данных.
USE_ATR_TARGETS = os.environ.get("USE_ATR_TARGETS", "false").strip().lower() == "true"
ATR_TARGET_MULTIPLIER = float(os.environ.get("ATR_TARGET_MULTIPLIER", "3.0"))

# --- P2.5: фильтр по перцентилю ATR (strategies.calc_atr_series,
# scanner._atr_percentile_exceeded) ---
# Если текущий ATR монеты (волатильность) находится выше этого
# перцентиля относительно её собственных ДНЕВНЫХ ATR за последние
# ATR_PERCENTILE_LOOKBACK_DAYS дней - сигнал по ней отсеивается ДО
# постановки в очередь: аномально высокая для КОНКРЕТНОЙ монеты
# волатильность (в моменте) делает и стопы, и тейки менее
# предсказуемыми - слиппедж и вероятность "стоп-охоты" выше, чем
# предполагает обычная логика сигнала, рассчитанная на её же обычный
# режим волатильности. Перцентиль СВОЙ для каждой монеты (не общий
# порог по всем парам сразу) - у BTC и у мелкой альты совершенно разный
# нормальный уровень ATR, единая абсолютная планка не имела бы смысла.
#
# Именно ДНЕВНЫЕ свечи, а не те же 15m, на которых строятся сами
# сигналы: чтобы получить 30-дневную историю на 15m, пришлось бы тянуть
# ~2900 дополнительных свечей на КАЖДУЮ монету при каждом тике сканера -
# непропорционально дорого по API-лимитам ради, по сути, макро-фильтра
# волатильности. Дневных данных для этой цели достаточно.
