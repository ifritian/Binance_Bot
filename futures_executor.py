...
def _emergency_close_or_raise(client: FuturesClient, symbol: str, step: str, original_error: Exception) -> None:
    ...
    Перед закрытием отменяет ЛЮБОЙ уже поставленный algo-ордер по
    символу (см. futures_client.cancel_all_algo_orders) - если сюда
    попали из-за отказа ПОСТАВИТЬ ТЕЙК, стоп-лосс к этому моменту уже
    успешно стоит на бирже с closePosition=true. Без явной отмены он
    остался бы висеть даже после аварийного закрытия позиции (Binance
    не отменяет closePosition-ордера сам, когда позиция обнуляется
    другим путём - см. docstring futures_position_monitor.py про то же
    самое для штатного закрытия) и мог бы неожиданно сработать на
    СЛЕДУЮЩЕЙ, никак не связанной позиции по этому же символу, если
    цена когда-нибудь снова коснётся того уровня. Отмена - best-effort:
    если сама не удалась, аварийное закрытие позиции всё равно
    продолжается (потеря защиты открытой позиции - более срочный риск,
    чем осиротевший ордер, который просто предупреждаем в логе)."""
    ...
    try:
        ...
        logger.warning(
            "Не удалось отменить уже выставленные ордера по %s перед аварийным закрытием (%s) - "
            "продолжаю закрытие позиции, но проверь вручную, не остался ли висеть старый стоп/тейк",
            symbol, e,
        )
    try:
